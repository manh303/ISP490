...
# Configurable table name (allows schema prefix). Default aligns with setup-activity-logs endpoint.
ACTIVITY_LOG_TABLE = os.getenv("ACTIVITY_LOG_TABLE", "user_activity_logs")
...
class ActivityLogger:
    ...
    async def log_activity(
        ...
        try:
            ...
            if "does not exist" in str(e).lower() or "undefined_table" in str(e):
                logger.warning(f"Activity log table missing ({self.table}); skipping log.")
                return
    ...
    async def get_activity_logs(
        ...
        try:
            ...
            count_query = f"SELECT COUNT(*) FROM {self.table} l {where_clause}"
            ...
            if "does not exist" in str(e).lower() or "undefined_table" in str(e):
                logger.warning(f"Activity log table missing ({self.table}); returning empty result.")
    ...
    async def get_log_detail(self, log_id: int):
        ...
        try:
            ...
            if "does not exist" in str(e).lower() or "undefined_table" in str(e):
                logger.warning(f"Activity log table missing ({self.table}); returning None.")
                return None
    ...
    async def get_activity_stats(self, days: int = 7):
        ...
        try:
            ...
            if "does not exist" in str(e).lower() or "undefined_table" in str(e):
                logger.warning(f"Activity log table missing ({self.table}); returning empty stats.")
                return {"daily_activity": [], "action_breakdown": [], "top_users": []}
    ...
    async def export_logs(
        ...
        try:
            ...
            if "does not exist" in str(e).lower() or "undefined_table" in str(e):
                logger.warning(f"Activity log table missing ({self.table}); export empty CSV.")
                return ""
