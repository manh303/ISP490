...
@router.get("/test/admin-token",
           summary="🔑 Get Admin Token (START HERE)",
           description="**STEP 1**: Get admin token for testing. Use this token in Authorize button!",
           tags=["🚀 Authentication"])

@router.get("/test/database",
           summary="🔍 Test Database Connection",
           description="Test direct database connection and query",
           tags=["🚀 Authentication"])
async def get_admin_token_for_testing():
    """
    📝 Get admin token for testing (Development only)
    
    **Usage**:
    1. Call this endpoint to get admin token
    2. Copy the token
    3. Use it in Authorization header: `Bearer <token>`
    4. Test other admin endpoints
    ...
    **Note**: This endpoint is for development/testing purposes only
    ...
    try:
        ...
        return {
            "success": True,
            "message": "Admin token for testing",
            "data": {
                "access_token": token,
                "token_type": "bearer",
                "usage": "Add this to Authorization header: Bearer " + token[:50] + "...",
                "admin_credentials": {
                    "email": "admin@dss.com",
                    "password": "admin123"
                },
        ...
        logger.error(f"Get admin token error: {e}")
        return {
            "success": False,
...
async def test_database():
    """
    Test database connection and query users directly
    """
    try:
        import sys
        import os
        ...
        # Test direct query
        query = "SELECT user_id, email, full_name, status FROM iam_user WHERE status = $1"
        result = await db_manager.execute_query(query, ('active',))
