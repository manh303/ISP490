...
class LazadaCookieCrawler:
    ...
    def extract_products(self, page) -> List[Dict[str, Any]]:
        try:
            ...
            try:
                page.wait_for_selector('div.Bm3ON[data-qa-locator="product-item"]', timeout=15000)
            except:
    ...
    def crawl_category(self, browser, category: str, url: str, max_pages: int = 60) -> List[Dict[str, Any]]:
        ...
        page.set_default_navigation_timeout(180000)  # 180s cho tất cả navigation
        all_products = []
            ...
            for page_num in range(1, max_pages + 1):
                ...
                while retries < max_retries:
                    try:
                        ...
                        # Check if redirected to security/block page
                        if "security" in page.url.lower() or "blocked" in page.url.lower() or "verify" in page.url.lower():
                            print(f"{LOG_PREFIX} WARNING: Possible anti-bot block detected on page {page_num}")
                            retries += 1
                        ...
                        if retries >= max_retries:
                            raise
                        print(f"{LOG_PREFIX} Retry {retries}/{max_retries}: {retry_e}")
                        time.sleep(random.uniform(5, 10))
                
                if not page_loaded:
                    print(f"{LOG_PREFIX} Failed to load page {page_num} after retries")
                    break
