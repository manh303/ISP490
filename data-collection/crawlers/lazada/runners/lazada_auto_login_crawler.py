#!/usr/bin/env python3
"""
Lazada Auto Login Crawler - Login tự động với username/password
"""
import json
import time
import random
...
class LazadaAutoLoginCrawler:
    ...
    def auto_login(self, page, email, password):
        """Auto login với email/password"""
        try:
            ...
            print(f"{LOG_PREFIX} Login successful, cookies saved")
            return True
            
        except Exception as e:
            print(f"{LOG_PREFIX} Login error: {e}")
            return False
    ...
    def extract_products(self, page) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"{LOG_PREFIX} Found {len(elements)} elements")
    ...
    def crawl_category(self, browser, category: str, url: str, max_pages: int = 2) -> List[Dict[str, Any]]:
        ...
        if not has_cookies and LAZADA_EMAIL and LAZADA_PASSWORD:
            self.auto_login(page, LAZADA_EMAIL, LAZADA_PASSWORD)
    ...
    def run(self, max_pages=2):
        ...
        if not LAZADA_EMAIL or not LAZADA_PASSWORD:
            print(f"{LOG_PREFIX} Missing LAZADA_EMAIL or LAZADA_PASSWORD env vars")
            print(f"{LOG_PREFIX} Set: export LAZADA_EMAIL=your@email.com")
            print(f"{LOG_PREFIX} Set: export LAZADA_PASSWORD=yourpassword")
            return
...
def main():
    crawler = LazadaAutoLoginCrawler()
